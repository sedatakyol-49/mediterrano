...
@Component({
  selector: 'app-fruehstuecksmenue',
  standalone: true,
  imports: [CommonModule, IconBadgeComponent],
  template: `
    <section class="page-hero">
      <h1>Mediterrano Frühstücksmenü</h1>
      <p>„Frühstücken wie im Urlaub – täglich bei uns."</p>
    </section>
  ...
  styles: [`
    .page-hero {
      background: linear-gradient(135deg, #d69f5f 0%, #2f7d69 100%);
      color: white;
      text-align: center;
      padding: 4rem 1rem;
    }
...
export class FruehstuecksmenueComponent {
  private menuService = inject(MenuService);
  
  categories = computed(() => 
    this.menuService.getCategories()().filter(cat => 
      cat.id === 'fruehstueck' || cat.id === 'fruehstueck-extras'
    )
  );
}
